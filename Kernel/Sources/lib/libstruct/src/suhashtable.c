...
/**
 * @brief 64 bits hash function for string keys.
 *
 * @details This function computes the 64 bits hash function for string keys.
 * The algorithm used is the FNV-1a hash algorithm.
 *
 * @param[in] key The key to hash.
 * @return The computed hash is returned.
 */
inline static uint64_t shash_64(const char* key);

/**
 * @brief Sets the data for a given entry in the string hash table.
 *
 * @details Sets the data for a given entry in the string hash table. If the
 ...
 */
static OS_RETURN_E shashtable_set_entry(shashtable_t* table,
                                        const char* key,
                                        void* data);
 ...
 */
static void shashtable_rehash_entry(shashtable_t* table,
                                    shashtable_entry_t* entry);
 ...
 * @return The error status is returned.
 */
static OS_RETURN_E shashtable_rehash(shashtable_t* table, const float growth);
                                        ...
                                        void* data)
{
    ...
    {
        /* Found the entry */
        if(strcmp(table->entries[entry_index]->key, key) == 0)
        {
    ...
    if(table->entries[entry_index] == NULL)
    {
        table->entries[entry_index] =
            table->allocator.malloc(sizeof(shashtable_entry_t));
        if(table->entries[entry_index] == NULL)
        {
    ...
    /* Set the data */
    keylen = strlen(key);
    table->entries[entry_index]->key = table->allocator.malloc((1 + keylen) *
                                                               sizeof(char));
    if(table->entries[entry_index]->key == NULL)
    {
        table->allocator.free(table->entries[entry_index]);
        return OS_ERR_MALLOC;
    }

    strncpy(table->entries[entry_index]->key, key, keylen);
    table->entries[entry_index]->key[keylen] = 0;
    table->entries[entry_index]->data        = data;
    table->entries[entry_index]->is_used     = TRUE;
                                    ...
                                    shashtable_entry_t* entry)
{
          ...
          table->entries[entry_index]->is_used == TRUE)
    {
        ...
        if(strcmp(table->entries[entry_index]->key, entry->key) == 0)
        {
...
static OS_RETURN_E shashtable_rehash(shashtable_t* table, const float growth)
{
    ...
    size_t               new_capacity;
    size_t               old_capacity;
    shashtable_entry_t** new_entries;
    shashtable_entry_t** old_entries;
    ...
    new_entries = table->allocator.malloc(sizeof(shashtable_entry_t*) *
                                          new_capacity);
    if(new_entries == NULL)
    {
        return OS_ERR_MALLOC;
    ...
    memset(new_entries, 0, sizeof(shashtable_entry_t*) * new_capacity);
    ...
    for(i = 0; i < old_capacity; ++i)
    {
        if(old_entries[i] != NULL)
        {
            ...
            {
                shashtable_rehash_entry(table, old_entries[i]);
            }
            else
            {
                /* The entry was not used, we can free it */
                table->allocator.free(old_entries[i]->key);
                                ...
                                OS_RETURN_E* error)
{
    ...
    table = allocator.malloc(sizeof(shashtable_t));
    if(table == NULL)
    {
        if(error != NULL)
        {
    ...
    table->entries = allocator.malloc(sizeof(shashtable_entry_t*) *
                                      HT_INITIAL_SIZE);
    ...
    memset(table->entries, 0, sizeof(shashtable_entry_t*) * HT_INITIAL_SIZE);
...
OS_RETURN_E shashtable_destroy(shashtable_t* table)
{
    ...
    for(i = 0; i < table->capacity; ++i)
    {
        ...
        {
            table->allocator.free(table->entries[i]->key);
                           ...
                           void* data)
{
    ...
    /* Insert the entry */
    return shashtable_set_entry(table, key, data);
}
